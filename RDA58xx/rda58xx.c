...
/*
* compile cmd:
  arm-none-linux-gnueabi-gcc -o hciattach_rda5876 ./hciattach_rda5876.c ./rda58xx.c -LXXX -lgpio
  Usage:
  hciattach_rda5876  -s 115200 /dev/ttyS1 rda 115200 noflow
  or
  hciattach_rda5876  -s 115200 /dev/ttyS1 rda 115200 flow
*/
...
#define GPIO_USERSPACE_SUPPORT
#define BT_POWER_GPIO 16
...
#ifdef GPIO_USERSPACE_SUPPORT
    gpio_set_value(BT_POWER_GPIO,1);
    usleep(50000);
#else   
	int bt_on_off = 1;
...
#endif
#ifdef GPIO_USERSPACE_SUPPORT
   	gpio_set_value(BT_POWER_GPIO,0);
   	usleep(50000);
#else     
	int bt_on_off = 0;
	ioctl(fd, IOCTL_BT_DEV_CTRL, &bt_on_off);		
...
int RDABT_core_Intialization(int fd)
{
	...
	bt_fd = gpio_init();
	if(bt_fd >= 0){
		gpio_free(BT_POWER_GPIO);
        gpio_request(BT_POWER_GPIO,"bt_power");
        gpio_direction_output(BT_POWER_GPIO, 0);   
	}else{
